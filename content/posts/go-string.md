...
为了计算字符串长度，程序需要从首地址开始扫描，直到遇到 `\0`，时间复杂度为 $O(n)$。

### 字符串中间不能直接包含 `\0`

一旦字符串中间出现 `\0`，传统 C 字符串函数就会将它视为结束位置。
