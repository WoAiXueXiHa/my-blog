...
* **语言规范**定义的是可观察行为，例如 `string` 不可变、`len` 返回字节数、索引得到字节。
* **当前运行时实现**可以帮助理解性能和内存现象，例如字符串描述信息通常包含数据指针和长度。
* **编译器优化**可能改变某次编译后的分配和复制细节，但不能改变语言层面的可观察结果。

后文凡是讨论 `stringStruct`、底层共享、零拷贝或内存滞留，都是实现层面的分析；业务代码只能依赖规范保证的行为。
...
> `stringStruct` 是运行时内部结构，这里只用于理解底层模型。语言规范定义的是 `string` 的行为，并不承诺业务代码可以直接依赖、构造或修改该内存布局。
...
因此，在这种实现模型下，字符串切片通常具有 $O(1)$ 的时间复杂度和 $O(1)$ 的额外空间复杂度。
...
在共享底层数据的实现中，如果 `firstLine` 长期存活，那么原来 1 GB 的底层字节数据仍然保持可达状态。
...
`strings.Clone` 不是常规必需操作，而是处理特定内存生命周期问题的工具。只有当原字符串很大、子字符串很小、并且子字符串会长期存活时，额外复制才通常值得考虑。
...
从规范语义看，`[]byte(s)` 的结果必须是一个可修改的字节切片；修改结果不能反过来影响原字符串。为了满足这个语义，从常见实现角度，转换过程通常需要：
...
从规范语义看，`string(b)` 的结果必须在后续修改 `b` 后仍保持原值。为了满足这个语义，从常见实现角度，转换通常需要：
...
但业务代码不应该依赖某个具体 Go 版本“一定零复制”的实现细节，也不能把返回的字符串理解成可以通过共享缓冲区继续修改的可变视图。
...
| 子字符串截取通常 $O(1)$ | 常见实现只需创建新的地址和长度描述，但规范只保证结果值正确 |
| 小子串可能拖住大内存 | 共享底层数据的实现中，子串的地址指针仍然引用原字符串的底层字节块 |
| `string` ↔ `[]byte` 通常需要复制 | 一个不可变、一个可变，转换结果必须语义独立 |
| 循环 `+=` 可能退化 | 旧内容在每次拼接时被重复复制，总复制量二次增长 |
| `strings.Builder` 适合动态拼接 | 通过可增长缓冲区减少反复分配和复制 |

这个模型的边界同样值得记住：规范保证的是不可变字节序列的行为，不保证具体内存布局、底层共享方式或某次转换是否真的发生物理复制。`string` 保存的是字节，不是字符，不是 rune，也不是用户视觉上的完整字形。UTF-8 解码发生在 `for range`、`utf8` 包等更高层操作中。当需要按字符位置处理时，需要显式地转换为 `[]rune` 或使用字素簇级别的工具。
...
* [Go 语言规范：String types](https://go.dev/ref/spec#String_types)
* [Go 语言规范：Conversions to and from a string type](https://go.dev/ref/spec#Conversions_to_and_from_a_string_type)
